...
JSBool js_pluginx_protocols_ProtocolAds_setAdsListener(JSContext *cx, uint32_t argc, jsval *vp)
{
	...
	if (argc == 1) {
		...
		//cobj->setAdsListener(<#cocos2d::plugin::AdsListener *pListener#>);
...
void js_register_pluginx_protocols_ProtocolAds(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
		...
		JS_FN("setAdsListener", js_pluginx_protocols_ProtocolAds_setAdsListener, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
	};
